...
function normalizeItem(q: any, ts: number, i: number, fallbackType?: GeneratedQuestion['type']): GeneratedQuestion {
  const type = (String(q?.type ?? fallbackType ?? '').trim() || 'open_analysis') as GeneratedQuestion['type'];
  const text = pickTextLike(q);
  const imageUrl = q?.imageUrl ?? q?.image_url ?? undefined;
  const options = pickOptionsLike(q);
  const id = `q_${ts}_${i}_${type}`;

  if (type === 'multiple_choice') {
    return { id, type: 'multiple_choice', text, options: Array.isArray(options) ? [...options] : [], include: true };
  }
  if (type === 'true_false') {
    return { id, type: 'true_false', text, include: true };
  }
  if (type === 'open_exercise') {
    return { id, type: 'open_exercise', text, include: true };
  }
  return { id, type: 'open_analysis', text, imageUrl, options: Array.isArray(options) ? [...options] : undefined, include: true };
}
